...
int main(int argc, char *argv[]) {
    char *filepath = NULL;
    int lowesttemp = 0;
    int highest_temps[YEARS];
    month_t rainy_month[YEARS];
    ...
    lowesttemp = min_temp(array);
    fprintf(stdout, "\nlowest temp: %d \n", lowesttemp);

    highest_temp_year(array, highest_temps);
    for (unsigned int year = 0u; year < YEARS; ++year)
    {
        printf("la mayor temperatura del año %d fue: %d\n", (year+FST_YEAR), highest_temps[year]);
    }

    most_rain_year(array, rainy_month);
    for (unsigned int year = 0u; year < YEARS; ++year)
    {
        printf("el mes mas lluvioso del año %d fue: %d\n", (year+FST_YEAR), rainy_month[year]);
    }
